...
let frontend ~filename =
  ...
  let conf =
    Setup.conf "cc" [] [] [] (* disable_linemarkers *) false [] (* save_cpp *) None
  in
...
let load_mucore_ast file : unit Mucore.file =
  let markers_env, ail_prog, prog2 = handle_frontend_error (frontend ~filename:file) in
  match
    Core_to_mucore.normalise_file ~inherit_loc:true (markers_env, snd ail_prog) prog2
  with
  | Ok prog3 -> prog3
  | Error err ->
...
let analyse_file file =
  let cn = load_mucore_ast file in
  Analysis.analyse cn
